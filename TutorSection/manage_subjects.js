            ...
            if (teacherDocSnap.exists()) {
                ...
                await updateDoc(teacherDocRef, {
                    subjects: teacherDocSnap.data().subjects.map(s => {
                        if (s.subject === subject.subject && s.description === subject.description) {
                            return {
...
async function deleteSubject(subject) {
    try {
        ...
        const teacherDocSnap = await getDoc(teacherDocRef);
        if (teacherDocSnap.exists()) {
            const updatedSubjects = teacherDocSnap.data().subjects.filter(s => s.subject !== subject.subject || s.description !== subject.description);
...
async function addSubject(subject, description) {
    try {
        ...
        const teacherDocRef = doc(db, "teachers", uid);
        await updateDoc(teacherDocRef, {
            subjects: arrayUnion({ subject, description, id: Math.floor(Math.random() * 9000) + 1000 })
        });
